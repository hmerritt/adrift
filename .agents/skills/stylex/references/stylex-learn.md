...
- https://stylexjs.com/docs/learn/animation/defining-keyframes/
- https://stylexjs.com/docs/api/javascript/create/
- https://stylexjs.com/docs/api/javascript/props/
- https://stylexjs.com/docs/api/javascript/types/
...
StyleX is a compile-time styling system. Define styles in JavaScript/TypeScript, then let the compiler extract and optimize CSS.

Use these rules as defaults:

- Keep styles statically analyzable.
- Prefer composition over ad hoc inline style objects.
- Use design variables for reusable values.
- Keep dynamic values in explicit StyleX-safe constructs.
...
## TypeScript types in StyleX

StyleX exposes TypeScript utilities for style typing.

From API docs:

- `StyleXStyles`: general type for StyleX style props.
- `StaticStyles`: type for static style sets.
- `StyleXStylesWithout<T>`: style type excluding one or more style properties.

Use these to define strict component boundaries for `sx` props and keep style extension predictable.

The theming docs also include type-oriented variable workflows, including typed variable values and stricter variable assignment constraints.

## Static-analysis limits and fixes

StyleX compilation expects static analyzable style declarations.

When compile errors mention static-only constraints:

1. Remove non-static constructs from raw style objects.
2. Move runtime-dependent values into style functions with explicit inputs.
3. Prefer variables and themes for values that change by mode/context.
4. Confirm module resolution/aliases if style imports fail to resolve.

Frequent causes:

- Runtime-calculated object spread directly into style objects.
- Non-deterministic values in raw declarations.
- Import path alias mismatches between bundler and StyleX config.

## Practical conversion workflow

Use this sequence when converting CSS or inline styles:

1. Group existing declarations by component concern.
2. Extract reusable values into vars.
3. Build a `stylex.create` map with stable keys.
4. Replace class or inline style usage with `stylex.props`.
5. Add `sx` passthrough only for reusable components.
6. Keep overrides explicit and last in composition order.
7. Re-test visual and interaction states after conversion.
